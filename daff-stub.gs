...
if (typeof exports != "undefined") {
    ...
    // promote methods of coopy.Coopy
    for (f in window.coopy.Coopy) { 
	if (window.coopy.Coopy.hasOwnProperty(f)) {
...
function daff_sheets(c1, m1) {
  // Diff CSV files using Daff then return a produced html string 
  var daff = init_daff(); 
  var daff_csv = new daff.Csv();
  var table1 = daff_csv.makeTable(c1);
  var table2 = daff_csv.makeTable(m1);
  var alignment = daff.compareTables(table2,table1).align();
  var data_diff = [];
